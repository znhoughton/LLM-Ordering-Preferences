...
input_texts_alpha = (np.array(np.array_split(input_texts_alpha, n_batches)).tolist() #for some reason, I can't conver the np array to a list -- python seems to think it's already a list, so I force it to be an array then conver it to a list

input_texts_nonalpha = (np.array(np.array_split(input_texts_nonalpha, n_batches)).tolist() 
