...
-finding the position of ' and' (if there are multiple 'ands' it chooses the one that comes immediately after one of the binomial words. There's a slim chance this will break if there are multiple ands AND the word in the binomial is tokenized into subtokens. However, this is probability pretty unlikely).
...
write_csv(reading_time_data, 'reading_time_data.csv')
```
